use super::{
  map::*, AreaOfEffect, BlocksTile, CombatStats, Confusion, Consumable, InflictsDamage, Item,
...
pub fn spawn_room(ecs: &mut World, room: &Rect) {
  let mut moster_spawn_points: Vec<usize> = Vec::new();
...

  {
    ...
    for _i in 0..num_monsters {
      ...
      while !added {
        ...
        if !moster_spawn_points.contains(&idx) {
          moster_spawn_points.push(idx);
          added = true;
        }
      }
    }
  ...
  for idx in moster_spawn_points.iter() {
    let x = *idx % MAPWIDTH;
    let y = *idx / MAPWIDTH;
